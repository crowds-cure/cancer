...
Template.selectTree.onRendered(() => {
  ...
  instance.useTransform = OHIF.ui.styleProperty.check(
    'transform',
    'translate(1px, 1px)'
  );
  ...
  instance.updateHeight = _.throttle(
    searchTerm => {
      let height;
  ...
  const selector =
    '.select-tree:first>.tree-content>.tree-options>.tree-breadcrumb .path-link';
  instance
    .$(selector)
    .css('width', 0)
    .each((index, element) => {
...
Template.selectTree.events({
  ...
  'change .select-tree:first>.tree-content>.tree-options>.tree-inputs>label>input'(
    event,
    instance
  ) {
      ...
      const getPosition = $element => {
        if (instance.useTransform) {
          ...
          const transformMatrix =
            matrixToArray($element.css('transform')) || [];
          return {
  ...
  'click .select-tree:first>.tree-content>.tree-options>.tree-breadcrumb .tree-back'(
    event,
    instance
  ) {
    const $target = $(event.currentTarget);
